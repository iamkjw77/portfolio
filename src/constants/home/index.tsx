  ...
  PROJECTS: [
    {
      ...
      background: [
        '이전에 채팅 기능을 구현한 경험이 있었지만, 비즈니스 로직과 UI 로직이 제대로 분리되지 않아 코드의 가독성과 유지보수성이 떨어지는 문제가 있었습니다. 이를 개선하기 위해 보다 효율적인 구조를 고민하게 되었고, 이를 실현하고자 채팅 관련 프로젝트에 참여하게 되었습니다.',
      ],

      meaning: [
        {
          title: '새로운 방식의 웹소켓 연결방식에 대한 고민',
          description: [
            '이전 프로젝트에서는 채팅방 입장 시 웹소켓을 연결하고, 나가면 연결이 종료되는 구조였습니다. 하지만 이번 친구하자 프로젝트에서는 사이드바에서 참여한 채팅방 목록을 유지하면서, 각 채팅방의 새로운 메세지를 실시간으로 업데이트해야 했습니다. 이를 위해 입장한 채팅방과 참여한 채팅방의 메세지를 하나의 웹소켓에서 수신하는 방식을 채택했습니다.',
            '이 과정에서 각 채팅방의 메세지를 적절히 구분하여 처리할 수 있도록, 웹소켓 메세지 이벤트가 발생할 때 구독된 모든 콜백 함수가 실행되는 구조를 설계했습니다. 이를 통해 입장한 채팅방과 참여 중인 채팅방의 메세지를 동시에 수신하고 처리할 수 있도록 구현했습니다.',
          ],
        },
        {
          title: '실시간 알림 기능의 필요성 및 개선 방향',
          description: [
            '웹소켓을 활용해 실시간 알림 기능을 구현했지만, 현재 알림이 필요한 상황은 친구 요청과 채팅방 초대 두 가지 뿐이었습니다. 참여한 채팅방의 메세지는 사이드바에서 실시간으로 업데이트되기 때문에 실시간 알림이 굳이 필요하지 않다는 생각이 들었습니다.',
    ...
    },
    {
      ...
      summary: [
        ...
        '다양한 이미지 입력을 위해 기존 에디터(tiptap) 커스터마이징',
        'FullCalendar를 활용한 일정 관리 시스템 개발 (스케줄 등록, 반복 일정, 폐강 기능)',
        '서비스 운영을 위한 기능(결제, 정산, CS 관리 등) 개발',
      ],
      background:
        '웰니스북은 운동 전문가들이 검증한 운동 프로그램을 제공하는 플랫폼입니다. 기존에는 판매자가 상품을 등록하고 관리할 수 있는 별도의 인터페이스가 없었습니다. 그로 인해 운영팀이 수기로 상품등록 및 일정 관리를 전담해야 했고, 이를 위해 판매자가 직접 상품을 관리할 수 있는 환경이 필요했습니다. 타 오픈마켓에서 상품을 등록할 때 필요한 기본적인 옵션과 서비스에 맞는 옵션까지 모두 구현 해야하고, 빠르게 개발해야 하는 기능이었기에 해결해야 할 과제가 많았지만, 그만큼 배울 점도 많았습니다.',
      meaning: [
        {
          title: '판매자가 직접 상품을 등록할 수 있는 시스템 개발',
          description: [
            '상품(운동 프로그램) 특성상 다양한 이미지 사이즈가 필요했고, 통일된 타이포가 필요했기 때문에 기존 에디터들 중에서 저희 서비스와 핏한 에디터는 찾아볼 수 없었습니다. 그래서 여러 에디터를 비교해서 tiptap이라는 에디터를 찾아냈고, 해당 에디터를 커스텀하여 상품 상세 설명 작성 시 저희 서비스에 맞는 다양한 옵션을 제공했습니다.',
        ...
        },
        {
          ...
          description: [
            '운영팀이 매출 데이터를 쉽게 관리할 수 있도록 결제 및 정산 시스템 구현했고, 각 기능마다 운영팀이 원하는 도구(엑셀)로 데이터를 다운로드 받을 수 있게하여 데이터를 관리를 편리하게 할 수 있도록 하였습니다.',
            '그 외 회원관리, CS 관리, 앱 내 컨텐츠 관리, 앱 푸시 알림 관리 등의 기능을 개발하였습니다.',
          ],
        },
        {
          title: '일정의 객관화',
    ...
    },
    {
      ...
      media: [{ type: 'image', url: '', thumbnail: '' }],
      summary: [
        '자주 변경되는 이벤트 화면으로 앱 심사 지연으로 이벤트 시작 일시를 놓치는 문제 발생',
        '변경이 잦은 화면(선물함, 쿠폰함 등)을 웹뷰로 제작해 빠른 배포 가능',
        '웹뷰에서도 앱과 동일한 기능을 사용할 수 있도록 자바스크립트 인터페이스 기반의 브릿지 개발',
        '앱 미설치 고객을 위한 웹 공유용 랜딩페이지 제공',
      ],
      background: [
        '웰니스북은 모바일 앱으로 제공되는 서비스입니다. 그렇다보니 변경이 잦은 화면을 엡으로 만들게 되는 경우 이벤트와 같이 정확한 일시에 시작해야 하는 기능 같은 경우 앱 심사가 길어지면 시작 일시를 놓치는 문제가 발생했습니다. 이러한 이유로 웹뷰로 만들어야 하는 화면이 필요해졌습니다.',
        '기존에는 사용자가 상품을 공유 받으면 앱을 설치해야만 상품 정보를 확인할 수 있었습니다. 이로 인해 앱을 설치하지 않은 고객이 상품 링크를 공유받았을 때 많은 경우 앱을 설치하지 않고 이탈하는 문제가 발생했습니다.',
      ],
      meaning: [
        {
          title:
          ...
          description: [
            '웹뷰에서 앱과 데이터를 주고받거나 앱 기능을 웹에서 활용해야 하는 경우가 많아, 자바스크립트 인터페이스 기반의 브릿지를 개발했습니다. 기존 국민피티(네이티브 앱)에서는 Android와 iOS의 메시지 함수를 각각 설정해야 했지만, 웰니스북(Flutter 앱)에서는 이를 하나의 메시지 함수로 통합하는 방법을 고민했습니다. 결과적으로 postMessage를 활용해 앱과의 데이터 송수신을 일관된 방식으로 구현할 수 있었습니다.',
          ],
        },
        {
    ...
    },
    {
      ...
      summary: [
        ...
        '웹에서도 OAuth 로그인(구글, 카카오, 애플) 및 본인 인증 구현',
      ],
      background: [
        '국민피티 서비스는 기존에 앱 중심으로 운영되었지만 웹을 통해서도 사용자들이 쉽게 정보를 얻고 서비스를 이용할 수 있도록, 본격적인 웹 플랫폼을 구축할 필요가 있었습니다. 초기 국민피티 웹사이트는 페이지에서 UI가 일관되지 않았고, 새로운 기능을 추가할 때마다 디자인 요소가 중구난방으로 적용되는 문제가 있었습니다. 또한, 메인 페이지는 서비스 소개를 위해 큰 이미지를 많이 포함하고 있어, 이로 인해 페이지 로딩 속도가 느려지는 문제도 있었습니다.',
      ],
      meaning: [
        {
          description: [
        ...
        {
          description: [
            '디자이너와 함께 디자인 시스템을 구축하면서, 컴포넌트의 동작과 사용 사례 등을 구체적으로 정의하고 문서화의 필요성을 느꼈습니다. 이를 위해 컴포넌트 문서화 기능이 있는 스토리북을 도입하였고, 스토리북을 통해 디자이너와 화면을 보며 피드백을 주고받을 수 있었습니다. 또한, 디자이너들이 각 컴포넌트를 자세히 검토할 수 있는 기회를 가지게 되어, 협업을 더 원활하게 진행할 수 있었습니다.',
          ],
        },
        {
          description: [
        ...
        {
          description: [
            '서비스를 제대로 소개하고 검색 노출을 원활하게 하기 위해 SEO 최적화를 진행했습니다. 이를 위해 이미지의 alt 속성, 메타 태그, 정적 페이지 생성 등을 적용하여 검색 엔진 노출을 강화하였습니다.',
          ],
        },
        {
          description: [
        ...
        {
          description:
            '이 외에도 웹에서도 OAuth 로그인(구글, 카카오, 애플) 및 본인 인증을 구현하여, 앱에서 제공되는 일부 기능을 웹에서도 사용할 수 있도록 하여 사용자 편의성을 강화했습니다.',
        },
      ],
    ...
    {
      id: 'kookminpt-business',
      name: '국민피티 비즈니스',
      period: '2021.07 - 2023.10',
      description:
        '개인 운동 강사와 운동 센터가 입점 신청을 통해 지역 기반으로 고객을 매칭받고, 운동 관련 사업을 운영 및 관리할 수 있도록 지원하는 웹 사이트(국민피티 파트너)',
      techStack: [
        'React',
        'Typescript',
        'Next.js',
      ...
      summary: [
        ...
        '복잡한 폼 관리 위해 react-hook-form 도입, 폼 상태 관리 간소화 및 개발 시간 단축',
        '피드형 콘텐츠를 위해 Intersection Observer로 무한 스크롤 구현',
      ],
      background: [
        '국민피티 서비스에는 파트너(개인 운동 강사, 운동 센터)를 위한 관리 시스템이 존재하지 않아, 운영팀이 모든 입점 및 센터 관리를 수작업으로 진행해야 했습니다. 이러한 이유로 파트너가 플랫폼을 통해 입점 신청을 하고 입점된 센터를 관리할 수 있는 시스템이 필요했습니다.',
        '센터 관련 정보를 입력해야 하는 여러 페이지(입점 신청, 센터 정보 입력 등)에서 다양한 폼 형태가 요구되었고, 이로 인해 폼 상태 관리가 복잡해져 이 상태들을 쉽게 관리할 수 있는 방법을 고민하게 되었습니다.',
      ],
      meaning: [
        {
          description:
            '복잡한 상태 관리를 위해 여러 가지 방법을 찾아보다가 react-hook-form이라는 라이브러리를 알게 되었고, 이를 도입하게 되었습니다. 초반에는 모든 폼을 마이그레이션해야 해서 시간이 많이 소요되었지만, 사용하면서 폼 상태를 훨씬 더 쉽게 관리할 수 있었고, isValid, isDirty 등 제공되는 다양한 기능 덕분에 오히려 그 이후에는 개발 시간이 단축되었습니다.',
        },
        {
          description:
            '센터 관리 기능 중 하나인 &quot;센터 새소식&quot;은 피드형 콘텐츠로, 무한 스크롤 기능이 포함된 페이지였습니다. 무한 스크롤 구현 방법을 학습한 후, 라이브러리 없이 Intersection Observer를 활용하여 무한 스크롤을 구현할 수 있었습니다.',
        },
      ],
    ...
    },
    {
      ...
      media: [{ type: 'image', url: '', thumbnail: '' }],
      summary: [
        '딥링크를 사용하여 앱 설치 고객은 앱 내 화면으로, 미설치 고객은 랜딩 페이지로 유도',
        '앱 내 업데이트가 많은 화면을 웹뷰로 제작하여 사용자 경험 강화.',
      ],
      background: [
        '앱 내에서 센터나 강사를 공유할 수 있는 기능이 있었지만, 처음에는 앱을 설치한 사용자만 이 기능을 이용할 수 있었습니다. 이로 인해 앱을 미설치한 고객은 강사나 센터 정보를 확인할 수 없었고, 이탈률이 높아지는 문제가 발생했습니다.',
    ...
    },
    {
      ...
      period: '2021.07 - 2023.10',
      description:
        '데이터 시각화를 위한 대시보드와 회원, 센터(파트너), 앱 내 콘텐츠, CS 등을 관리할 수 있도록 개발된 백오피스',
      techStack: [
        'React',
        'Typescript',
        'Next.js',
